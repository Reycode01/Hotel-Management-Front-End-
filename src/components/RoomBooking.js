...
const RoomBooking = () => {
  ...
  const fetchRoomBookings = async () => {
    try {
      const response = await axios.get('https://hotel-management-backend-1-55vk.onrender.com/api/room-bookings', {
        params: { bookingDate }
      });
  ...
  const handleBooking = async () => {
    ...
    try {
      const response = await axios.post('https://hotel-management-backend-1-55vk.onrender.com/api/room-bookings', {
