...
const Salaries = () => {
  ...
  useEffect(() => {
    const fetchSalaries = async () => {
      try {
        const { data } = await axios.get('https://hotel-management-backend-1-55vk.onrender.com/api/salaries');
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post('https://hotel-management-backend-1-55vk.onrender.com/api/salaries', newSalary);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://hotel-management-backend-1-55vk.onrender.com/api/salaries/${id}`);
  ...
  return (
    <div className="flex flex-col items-center bg-gray-900 py-4 px-4">
      ...
      {salariesList.length > 0 && (
        <div className="mt-12 p-3 bg-gray-800 bg-opacity-90 rounded-lg shadow-lg w-full border border-gray-700 backdrop-blur-md">
          <h3 className="text-2xl font-semibold mb-3 text-teal-300">Salaries List</h3>
