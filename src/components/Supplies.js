...
const Supplies = () => {
  ...
  const fetchSupplies = async () => {
    try {
      const response = await axios.get(`https://hotel-management-backend-7.onrender.com/api/supplies`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post('https://hotel-management-backend-7.onrender.com/api/supplies', newSupply);
  ...
  const handleDelete = async (supplyId) => {
    try {
      await axios.delete(`https://hotel-management-backend-7.onrender.com/api/supplies/${supplyId}`);
